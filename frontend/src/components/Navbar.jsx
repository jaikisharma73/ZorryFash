...
const Navbar = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (isHome) {
                // Show sticky after scrolling past the hero height
    ...
    // Determine tag colors based on scroll state
    const tagColor = scrolled ? 'text-white' : 'text-black';
    const tagHover = scrolled ? 'text-black' : 'text-white';

    return (
        <div className={`z-50 flex items-center justify-between pb-5 pt-[10px] font-medium ${isHome ? 'fixed top-0 left-0 w-full px-[8px] z-50' : 'relative -mx-4 sm:-mx-[5vw] md:-mx-[7vw] lg:-mx-[9vw] px-[8px]'} ${scrolled ? 'bg-black shadow-sm' : 'bg-transparent'} transition-colors duration-300`}>
